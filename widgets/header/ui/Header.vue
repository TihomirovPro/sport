...
<template>
<header class="px-2 py-1 sticky top-0 bg-accent shadow-md z-50">
  <div class="max-w-2xl mx-auto flex flex-wrap items-center justify-between">
    <div
      v-if="isShowBackBtn"
      class="size-8 text-l flex-center header-soft-text"
      @click="back"
    >
      <div class="gg-push-chevron-left"></div>
    </div>
    <div
      v-else
      class="size-8 flex-center flex-col gap-1"
      @click="toSettings"
    >
    ...
    <div class="text-l header-soft-text">{{ headerTitle }}</div>
    <span
      v-if="!isOnline || pendingOperations"
      class="text-xs offline-badge"
    >{{ offlineStatus }}</span>

    <div class="flex-center size-8" @click="addItem">
      <div class="w-4 bg-white absolute rounded h-0.5"></div>
      <div class="h-4 bg-white absolute rounded w-0.5"></div>
    </div>
  </div>
</header>
</template>
