...
const { t, locale, translateEase, getIntlLocale } = useI18n()
...
const formatDate = computed(() => {
  ...
  return locale.locale === 'ru' ? formatted.replace(' г.', '') : formatted
})
...
<div class="grid w-full py-4 px-3 text-xs border border-faint rounded-xl shadow-md bg-faint/20">
  <div class="flex items-center justify-between text-accent pb-6">
    <p>{{ formatDate }}</p>
    <p v-if="isComplex">{{ t('workout.timed') }}{{ rounds ? ` · ${t('workout.rounds', { count: rounds })}` : '' }}</p>
    <p v-else>{{ approach.length }} x {{ interval }}</p>
    <p v-if="!isComplex">{{ ease === EnumEase.rubber ? rubber : translateEase(ease) }}</p>
  </div>

  <div class="grid" :style="cols" v-if="!isComplex">
    <div class="text-left py-1 pr-1 text-text/40">{{ t('common.reps') }}</div>
    ...
    <template v-if="ease === EnumEase.weight && Array.isArray(weight) && weight.length">
      <div class="text-left pr-1 py-1 border-t border-faint text-text/40">{{ t('common.kg') }}</div>
  ...
  <div class="grid items-center gap-3 grid-cols-[1fr_auto]" v-else>
    <div class="text text-text/50">{{ t('workout.time') }}</div>
  ...
  <div class="border-t border-faint p-2 mt-2" v-if="isComplex && Array.isArray(complexExercises) && complexExercises.length">
    <div class="text pb-2 text-text/50">{{ t('workout.statsExercises') }}</div>
  ...
  <div class="flex justify-end pt-3">
    <button type="button" :aria-label="t('workout.editWorkout')" @click="selectUpdate">
